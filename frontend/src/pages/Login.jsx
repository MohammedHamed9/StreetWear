    ...
    async function handelMerge() {
      if (user) {
        try {
          ...
          navigate(redirectPath == "checkout" ? `/${redirectPath}` : "/", {
            replace: true,
          });
        } catch (error) {
  ...
  return (
    <div className="flex">
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center items-center p-8 md:p-12 ">
        ...
        >
          <h2 className="text-xl font-sans font-medium text-center mb-4">
            StreatWear
          </h2>
          ...
          <button
            type="submit"
            className="bg-black text-white text-sm w-full py-3
             rounded-lg mb-6 hover:bg-gray-800 transition"
          >
            {loading ? "Sgining in..." : "Sgin in"}
          </button>
          <p className="text-center text-sm font-medium">
            Don't have an account?
            <Link
              to={redirect ? "/register?redirect=checkout" : "/register"}
